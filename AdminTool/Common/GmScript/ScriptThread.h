...
public:
    ...
    int SendByHandle(uint64_t dest, const void* data, int len)
    {
        int ret = 0;
        if (NULL != data) {
            if (NULL != m_pSession) {
    ...
    int SendByName(const char* name, const void* data, int len)
    {
        ...
        if (NULL != name && NULL != data) {
            uint64_t dest = TargetHandle(name);
            ret = SendByHandle(dest, data, len);
        }
        return ret;
    }
    int SendCommandByHandle(uint64_t dest, const char* result)
    {
        int ret = 0;
        if (NULL != result) {
            if (NULL != m_pSession) {
                int cmdSize = static_cast<int>(strlen(result));
                MessageCommand msg;
                msg.m_Src = MyHandle();
                msg.m_Dest = dest;
                if (m_pSession->Send(reinterpret_cast<const char*>(&msg), static_cast<int>(sizeof(msg) - 1), result, cmdSize + 1)) {
                    ret = 1;
                }
            }
        }
    ...
    virtual ExecuteResultEnum ExecuteCustomMember(int index, int paramClass, Value* pParams, int num, Value* pRetValue)
    {
        if (NULL != pParams) {
            ...
            case CUSTOM_MEMBER_INDEX_MYHANDLE:
                if (NULL != pRetValue) {
                    uint64_t r = m_Client.MyHandle();
                    pRetValue->SetUInt64(r);
                }
                break;
...
public:
    ...
    {}
    SharedMessageQueue& GetSharedMessageQueue() { return m_SharedMessageQueue; }
private:
    void Reset()
    {
