...
namespace ScriptableData
{
  ...
  public:
    Function(ScriptableDataFile& dataFile);
  ...
  public:
    Statement(ScriptableDataFile& dataFile);
  ...
  public:
    INT		        GetScriptableDataNum(void)const{ return m_ScriptableDataNum; }
    Statement*    GetScriptableData(INT index)const
    {
      if (index < 0 || index >= m_ScriptableDataNum)
        return NULL;
      return m_ScriptableDatas[index];
    }
  ...
  public:
    ScriptableDataFile(void);
    ScriptableDataFile(const ParserOptions& options);
    ~ScriptableDataFile(void);
    void				  Reset(void);
    void          ParseText(const char* buf);
    void          ParseText(IScriptSource& source);
    void          ParseBinary(const CHAR* buf, INT size);
  private:
    void				  Init(void);
    void				  Release(void);
  private:
    ...
    StatementPtr*	m_ScriptableDatas;
    INT					  m_ScriptableDataNum;
  public:
