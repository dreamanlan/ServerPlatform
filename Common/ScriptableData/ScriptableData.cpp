...
namespace ScriptableData
{
  ...
  Function::Function(ScriptableDataFile& dataFile) :ISyntaxComponent(ISyntaxComponent::TYPE_FUNCTION),
    m_Statements(0),
    m_StatementNum(0),
    m_StatementSpace(0),
  ...
  Statement::Statement(ScriptableDataFile& dataFile) :ISyntaxComponent(ISyntaxComponent::TYPE_STATEMENT),
    m_Functions(0),
    m_FunctionNum(0),
    m_FunctionSpace(0)
  {
  ...
  void ScriptableDataFile::AddStatement(Statement* p)
  {
    if (0 == p || 0 == m_ScriptableDatas)
      return;
    if (m_ScriptableDataNum < 0 || m_ScriptableDataNum >= m_Options.GetMaxProgramSize())
      return;
    m_ScriptableDatas[m_ScriptableDataNum] = p;
    ++m_ScriptableDataNum;
  }

  Value* ScriptableDataFile::AddNewValueComponent(void)
  {
    Value* p = new Value();
  ...
  Call* ScriptableDataFile::AddNewCallComponent(void)
  {
    Call* p = new Call();
    AddSyntaxComponent(p);
    return p;
  ...
  Function* ScriptableDataFile::AddNewFunctionComponent(void)
  {
  ...
  Statement* ScriptableDataFile::AddNewStatementComponent(void)
  {
  ...
  void ScriptableDataFile::AddSyntaxComponent(ISyntaxComponent* p)
  {
  ...
  CHAR* ScriptableDataFile::AllocString(INT len)
  {
  ...
  CHAR* ScriptableDataFile::AllocString(const CHAR* src)
  {
    if (0 == src)
      return 0;
  ...
  ScriptableDataFile::ScriptableDataFile(void) :m_IsDebugInfoEnable(FALSE),
    m_StringBuffer(NULL),
    m_UnusedStringPtr(NULL),
    m_SyntaxComponentPool(NULL),
    m_ScriptableDatas(NULL)
  {
    Init();
  }

  ScriptableDataFile::ScriptableDataFile(const ParserOptions& options) :m_Options(options), m_IsDebugInfoEnable(FALSE),
    m_StringBuffer(NULL),
    m_UnusedStringPtr(NULL),
    m_SyntaxComponentPool(NULL),
    m_ScriptableDatas(NULL)
  ...
  ScriptableDataFile::~ScriptableDataFile(void)
  {
    Release();
  }

  void ScriptableDataFile::Reset(void)
  {
    Release();
    Init();
  }

  void ScriptableDataFile::ParseText(const char* buf)
  {
    ScriptableData::ParseText(buf, *this);
  }

  void ScriptableDataFile::ParseText(IScriptSource& source)
  {
    ScriptableData::ParseText(source, *this);
  }

  void ScriptableDataFile::ParseBinary(const CHAR* buf, INT size)
  {
    ScriptableData::ParseBinary(buf, size, *this);
  }
  
  void ScriptableDataFile::Init(void)
  {
    ...
    m_ScriptableDatas = new StatementPtr[m_Options.GetMaxProgramSize()];
    m_ScriptableDataNum = 0;
    m_ErrorAndStringBuffer.Reset(m_StringBuffer, m_UnusedStringPtr, m_Options.GetStringBufferSize());
  }

  void ScriptableDataFile::Release(void)
  {
    if (0 != m_StringBuffer) {
    ...
    if (0 != m_ScriptableDatas) {
      delete[] m_ScriptableDatas;
      m_ScriptableDataNum = 0;
    }
  }
  ...
  void ScriptableDataFile::WriteToFile(FILE* fp, int indent)const
  {
#if _DEBUG
    for (int ix = 0; ix < GetScriptableDataNum(); ++ix){
      Statement* pStatement = GetScriptableData(ix);
      if (pStatement){
