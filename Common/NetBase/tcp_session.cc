            ...
            do {
                ...
                try {
                    ...
                    if (!recv_cb.isNull()) {
                        ...
                        (recv_cb)(impl->session, data, static_cast<int>(size));
                    }
                }
                catch (...) {}
...
namespace net_base
{
    ...
    bool TcpSession::Send(evbuffer* evbuf)
    {
        ...
        u_long nsize = htonl(static_cast<u_long>(size));
        bufferevent_lock(m_Impl->bev);
        if (0 != bufferevent_write(m_Impl->bev, static_cast<const void*>(&nsize), sizeof(nsize)) ||
