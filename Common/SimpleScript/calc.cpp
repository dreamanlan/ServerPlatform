...
namespace FunctionScript
{
  ...
  namespace InnerApi
  {
    ...
    public:
      virtual ExecuteResultEnum Execute(Value* pRetValue)const
      {
        //函数定义生成一个Closure实例，目前未实现capture功能。
        ...
        if(0!=pClosure)
        {
          pClosure->SetDefinitionRef(statement);
          if(func0->HaveParam())
          {
    ...
    public:
      explicit FunctionStatement(Interpreter& interpreter):StatementApi(interpreter),m_pDefine(NULL){}
    private:
      Statement* m_pDefine;
    };
    ...
    public:
      virtual int IsMatch(const Statement& statement)const
      {
        //函数定义语法：function(name)capture(@a,@b,@c){};其中name与capture(@a,@b,@c)可选
        int num=statement.GetFunctionNum();
        if(1!=num && 2!=num)
          return FALSE;
        Function* pFunc0=statement.GetFunction(0);
        ...
        else
        {
          ...
          if(0!=pFunc1)
          {
            ...
            if(funcName1.IsVariableName())
            {
              if(0==funcName1.GetString() || 0!=strcmp(funcName1.GetString(),"capture"))
                return FALSE;
            }
            else
            {
  ...
  //------------------------------------------------------------------------------------------------------
  void Closure::SetDefinitionRef(const Statement& statement)
  {
